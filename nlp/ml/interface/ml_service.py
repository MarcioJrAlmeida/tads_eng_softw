    ...
    try:
        ...
        cursor.execute("""
            SELECT  
                ...
                r.id_resposta
            FROM Avaliacao AS a
            LEFT JOIN Contem AS c ON a.id_avaliacao = c.id_avaliacao
            LEFT JOIN Pergunta AS p ON c.id_pergunta = p.id_pergunta
            LEFT JOIN Resposta AS r ON a.id_avaliacao = r.idAvaliacao
...
def listar_frases_suspeitas():
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            SELECT TOP 10 id, id_avaliacao, contexto_pergunta, conteudo_resposta
